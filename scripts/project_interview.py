#!/usr/bin/env python3
"""Utilities for the ASu /project-interview skill.
...
def build_prompt(
    ...
    lines: list[str] = [
        "请按 `/project-interview` 执行：基于当前项目仓库或以下项目材料，",
        "在目标项目根目录写入 `导学-{简称}.md` 与 `面经-{简称}.md`，",
        "并在最终回复中整理可交接给 `/asu` 与 `/interview` 的证据摘要。",
        "",
        "## 已确认输入",
        "",
        "### 项目简称（用于文件名）",
    ...
    lines.extend(
        [
            ...
            "- 面经：包含 1-2 句项目简介、4-6 条简历 bullet、15-25 个主问题和追问口播。",
            "- 口播：第一人称，主问和追问均不少于 150 个汉字，覆盖 STAR。",
            "- 证据：源码路径、函数名和内部名词集中放到「源码证据索引」，正文用通用工程语言表达。",
            "- 交接：最终回复附 `/asu` 项目事实摘要和 `/interview` 高风险 Claim 清单。",
            "",
        ]
    )
    return "\n".join(lines)
...
def command_check(args: argparse.Namespace) -> int:
    ...
    print(f"[{status}] project-interview input check\n")
    if result.missing:
        print("必须补齐：")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Utilities for ASu /project-interview.")
    ...
    prompt_parser = subparsers.add_parser("build-prompt", help="Build a /project-interview prompt.")
