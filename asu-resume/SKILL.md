---
name: asu-resume
description: 中文可编辑简历制作技能：根据用户经历选择或复刻模板，生成可编辑 HTML 简历并提供 PDF 导出；当用户要求制作、修改、复刻简历文件，或说"帮我做简历""生成简历 HTML""导出 PDF 简历"时使用。
---
...
本技能自带后备模板 `assets/resume-editable.html`（简洁 A4 可编辑单/双栏模板，含打印隐藏工具栏与 PDF 导出能力）。优先使用该后备模板；若后续用户提供了官方 18 套模板资源，可替换使用。使用后备模板时，在交付说明中写明"当前使用后备模板，视觉风格较少、不保证与官方模板完全一致"，不因为资源缺失而静默降级，也不把缺失资源重新伪造为"已使用官方模板"。

## 工作流程

1. 先按"输入清单"盘点材料；用户提供主张—证据账本时，按 [账本状态规则](references/claim-evidence-ledger.md) 读取事实。缺少关键字段时按"缺失字段补问规则"补问，无法补齐时使用 `待补充` 或 `待确认` 占位，不捏造经历。
...
- 发现用户材料中有真实隐私信息时，只写入用户要求的简历文件，不写入技能模板、示例或 README。
...
PDF 不是简单"能保存"即可。本机装有 Edge/Chrome 时，可指导用户浏览器打印另存为 PDF。验收标准：
...
默认交付：可编辑 `.html` 文件、使用的模板说明、资源/fallback 状态、print-preview QA 结果、各页内容占用比例、浏览器编辑方法和 PDF 导出方法。若用户同时需要经历改写，先调用 asu-experience 完成文字，再将确认后的内容放入简历。
