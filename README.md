...
| `/project-interview` | 项目导学面经 | 基于项目仓库生成 `导学-{简称}.md`、`面经-{简称}.md` 和交接摘要 |
...
| 已有项目仓库，需要梳理源码阅读路径和面试口播 | `/project-interview` |
...
- **已有项目、准备开始投递**：先用 `/project-interview` 梳理项目学习路径和面经，再用 `/asu` 对齐目标岗位，最后用 `/make-resume` 或 `/asu-resume` 生成简历；
...
```text
请从这个 GitHub 仓库安装 ASu-skills 插件，并启用其中的 contributor、project-interview、asu、make-resume、asu-resume、interview、offer 七个 skills：
https://github.com/Hisn00w/ASu-skills
```

安装完成后建议新建一个 Codex 对话，让新 skills 被重新加载。然后在输入框中输入 `/`，从命令列表选择 `contributor`、`project-interview`、`asu`、`make-resume`、`asu-resume`、`interview` 或 `offer`。
...
$project-interview 基于当前项目生成导学和面经，并整理可交接给 /asu 与 /interview 的证据摘要。
...
3. 新建对话，在输入框输入 `/`，从命令列表选择 `contributor`、`project-interview`、`asu`、`make-resume`、`asu-resume`、`interview` 或 `offer`。
...
## `/project-interview`：项目导学面经

`/project-interview` 面向已有项目仓库或项目材料，负责生成两份可落盘的 Markdown：`导学-{简称}.md` 和 `面经-{简称}.md`。它会把源码阅读路径、技术亮点、设计取舍、STAR 口播和源码证据索引整理成面试前可复习的材料，并在结尾给出可交给 `/asu` 的项目事实摘要，以及可交给 `/interview` 的高风险 Claim 清单。
...
2. 用 `/project-interview` 把已有项目仓库整理成导学、面经和可追问证据；
...
也可以在同一条需求里说明组合目标，例如：“先用 `/project-interview` 生成项目导学和面经，再用 `/asu` 改写经历，最后用 `/make-resume` 生成 HTML 简历”。
...
│   │   └── agents/openai.yaml
│   ├── project-interview/
│   │   ├── SKILL.md             # /project-interview 项目导学面经
│   │   └── agents/openai.yaml
│   ├── make-resume/
