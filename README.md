...
<div align="center">
  ...
  <h3>中文求职工作流插件</h3>
  <p>用八个独立入口完成开源贡献、对话复盘、项目导学面经、经历酥化、简历制作、简历投递填写、面试准备和校招进度管理。</p>
</div>
...
```text
请从这个 GitHub 仓库安装 ASu-skills 插件，并启用其中的 contributor、asu-recap、project-guide、asu、make-resume、job-apply、interview、offer 八个 skills：
https://github.com/Hisn00w/ASu-skills
```

安装完成后建议新建一个 Codex 对话，让新 skills 被重新加载。然后在输入框中输入 `/`，从命令列表选择 `contributor`、`asu-recap`、`project-guide`、`asu`、`make-resume`、`job-apply`、`interview` 或 `offer`。
...
3. 新建对话，在输入框输入 `/`，从命令列表选择 `contributor`、`asu-recap`、`project-guide`、`asu`、`make-resume`、`job-apply`、`interview` 或 `offer`。
...
- **已有项目、准备开始投递**：先用 `/project-guide` 梳理项目学习路径和面经，再用 `/asu` 对齐目标岗位，最后用 `/make-resume` 生成简历；默认使用 ASu 模板，也可以指定其他模板；
