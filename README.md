...
ASu-skills 同时支持 Codex、Claude Code、Cursor、TraeWork 和 Qoder，另有 OpenCode 与 WorkBuddy 的轻量桥接入口。所有入口共用同一套 `skills/`、`assets/` 和 `references/`；入口清单以根目录 `skills.registry.json` 为单一事实源，由 `npm run sync:skills` 生成并对账（CI 以 `--check` 校验）。
...
| Cursor      | [查看安装说明](.cursor-plugin/install.md)                                                  | 新建 Agent 对话，输入 `/` 或 `/contributor` 等 | 删除项目 `.cursor/skills/` 或用户 `~/.cursor/skills/` 下对应目录 |
