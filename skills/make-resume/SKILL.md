...
1. 盘点用户消息、附件和项目文件中的姓名、联系方式、教育、实习/工作、项目、技能、目标岗位、公开链接、页数和照片需求。用户提供主张—证据账本时，按 [账本状态规则](../asu/references/claim-evidence-ledger.md) 读取事实。
...
默认交付可编辑 `.html` 文件、使用的模板说明、资源或 fallback 状态、print-preview QA 结果、各页内容占用比例、浏览器编辑与保存方法和 PDF 导出方法。若用户同时需要经历改写，先调用 `/asu` 完成文字，再将确认后的内容放入 `/make-resume` 生成的用户副本；不得修改默认 ASu 模板或用户指定的只读模板源文件。
