---
name: interview
description: 中文简历、目标岗位与面试时间驱动的面试准备技能：生成有优先级的学习计划，提取需要验证的经历 Claim，按面试轮次建立问题与评分契约，通过一次一问、证据反馈和弱项复练检查真实掌握程度；当用户输入“/interview”、要求制定面试计划、预测问题、模拟面试、简历追问、压力面、场景设计题或检查掌握程度时使用。
---

# /interview：把简历问穿

`/interview` 不替用户编造可以背诵的经历，而是在用户需要准备计划时，把面试时间、JD、简历与真实基础转成可执行的准备路径，并把重要 Claim 变成可验证的问题。它通过按需规划、面试契约、逐轮追问、证据评估和弱项复练，检查用户能否讲清真实职责、技术实现、指标口径、决策取舍和失败案例。
...
当用户要求制定准备计划、临近面试冲刺、专题讲解或输入 `/interview prepare` 时：

1. 计算从当前时间到面试的实际剩余时间，并扣除睡眠、工作、通勤等不可用时段；
2. 将 JD 硬要求、简历高风险 Claim、已暴露薄弱项和当前能力放入同一优先级矩阵；
3. 按剩余时间选择冲刺、半天、每日或阶段计划，不平均分配学习时间；
4. 每项任务写清练习方式、完成标准和预计耗时；
5. 使用“理解 → 准确表述 → 最小代码或操作 → 口述 → 小练习”开展专题教学；
6. 完成第一轮检查点或用户明确要求后，再进入 Grill。
...
`/contributor → /asu → /make-resume 或 /asu-resume → /interview prepare → /interview grill → /offer`

`/asu` 让真实经历表达得更强，`/interview` 确认这些表达是否接得住。简历写给 HR，追问留给事实。
