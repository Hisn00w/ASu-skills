...
project_interview = load_module("project_interview_module", "scripts/project_interview.py")


class ProjectInterviewTests(unittest.TestCase):
    def test_validate_short_name_accepts_valid_value(self):
        self.assertEqual(project_interview.validate_short_name("智能BI"), "智能BI")

    def test_validate_short_name_rejects_illegal_chars(self):
        with self.assertRaises(ValueError):
            project_interview.validate_short_name("bad/name")

    def test_validate_short_name_rejects_reserved_names(self):
        with self.assertRaises(ValueError):
            project_interview.validate_short_name("..")

    def test_analyze_short_description_requires_more_input(self):
        result = project_interview.analyze_input("太短了")
    ...
    def test_analyze_complete_description_has_note(self):
        result = project_interview.analyze_input(
    ...
    def test_build_prompt_contains_skill_outputs_and_handoffs(self):
        prompt = project_interview.build_prompt(
        ...
        self.assertIn("/project-interview", prompt)
    ...
    def test_main_check_prints_json_output(self):
        ...
        with mock.patch("sys.stdin", io.StringIO(description)):
            with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                exit_code = project_interview.main(["check", "--json"])
    ...
    def test_main_build_prompt_reads_json_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stderr = io.StringIO()
            with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                exit_code = project_interview.main(["build-prompt", "--json-file", str(json_file)])
        ...
        self.assertIn("/project-interview", output)
