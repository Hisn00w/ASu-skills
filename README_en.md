...
| `/project-interview` | Project interview prep | Generates `导学-{short-name}.md`, `面经-{short-name}.md`, and handoff evidence from a project repository |
...
| Have a project repository and need a code-reading path plus interview talking points | `/project-interview` |
...
- **Have projects, ready to apply**: use `/project-interview` to turn the repository into study notes and interview answers, then use `/asu` to align with the target role and `/make-resume` or `/asu-resume` to generate the resume;
...
```text
Install the ASu-skills plugin from this GitHub repository and enable the seven skills: contributor, project-interview, asu, make-resume, asu-resume, interview, offer:
https://github.com/Hisn00w/ASu-skills
```

After installation, start a new Codex conversation so the new skills get reloaded. Then type `/` in the input box and pick `contributor`, `project-interview`, `asu`, `make-resume`, `asu-resume`, `interview`, or `offer` from the command list.
...
$project-interview Generate project study notes and interview answers from the current repository, with handoff evidence for /asu and /interview.
$make-resume Turn my experience into an editable Chinese HTML resume. If needed, ask the agent to produce an English resume instead.
...
3. Start a new conversation, type `/` in the input box, and pick `contributor`, `project-interview`, `asu`, `make-resume`, `asu-resume`, `interview`, or `offer` from the command list.
...
- 18 Chinese HTML templates; ask your agent for an English version if needed;
...
2. Use `/project-interview` to turn an existing project repository into study notes, interview answers, and source evidence;
...
You can also state a combined goal in a single request, e.g.: “first use `/project-interview` to generate project study notes and interview answers, then `/asu` to rewrite the experience, and finally `/make-resume` to generate an HTML resume”.
...
│   │   └── agents/openai.yaml
│   ├── project-interview/
│   │   ├── SKILL.md             # /project-interview project study notes and interview answers
│   │   └── agents/openai.yaml
│   ├── make-resume/
